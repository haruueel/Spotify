...
public class LibraryViewController implements Initializable {
    ...
    private void displayArtistsInPane(){
        try {
            ResultSet query = SpotifyController.st.executeQuery("select artista_id from sigue_artista where usuario_id="+SpotifyController.getUser().getId()+";");
    ...
    private void displayAlbumsInPane(){
        try {
            ResultSet query = SpotifyController.st.executeQuery("select album_id from sigue_album where usuario_id="+SpotifyController.getUser().getId()+";");
            if (query.isBeforeFirst()) {
                while (query.next()) {
    ...
    private void displaySongsInPane(){
        try {
            ResultSet query = SpotifyController.st.executeQuery("select cancion_id from guarda_cancion where usuario_id="+SpotifyController.getUser().getId()+";");
            if (query.isBeforeFirst()) {
                while(query.next()) {
    ...
    private void displayPlaylistInPane(){
        try{
            ResultSet query = SpotifyController.st.executeQuery("select playlist_id from sigue_playlist where usuario_id="+SpotifyController.getUser().getId()+" and playlist_id like '___';");
            if (query.isBeforeFirst()){
                while (query.next()){
